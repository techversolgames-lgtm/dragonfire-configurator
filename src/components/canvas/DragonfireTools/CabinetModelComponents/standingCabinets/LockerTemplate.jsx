...
import CabinetFeet from "../CabinetFeet";
...
/**
 * Locker-only manual offsets for base options.
 * Tune these values while aligning the locker for each base option.
 */
const LOCKER_BASE_OPTION_OFFSETS = {
  riser: {
    bodyY: -0.16,
    feetInsetZFront: undefined,
    feetInsetZBack: undefined,
  },
  wheel: {
    bodyY: -0.064,
    feetInsetZFront: 0.0,
...
  /** Locker GLB includes built-in feet geometry in its local origin, so keep body at local Y=0. */
  const floorOffsetY = 0.066;
  const baseOptionOffsets = LOCKER_BASE_OPTION_OFFSETS[baseOption];
  const lockerBodyYOffset = baseOptionOffsets?.bodyY ?? 0;
...
        ref={cabinetRef}
      >
        <group position={[0, floorOffsetY + lockerBodyYOffset, 0]}>
...
          isGhost={isGhost}
          insetZFrontOverride={baseOptionOffsets?.feetInsetZFront}
          insetZBackOverride={baseOptionOffsets?.feetInsetZBack}
        />
      </group>
