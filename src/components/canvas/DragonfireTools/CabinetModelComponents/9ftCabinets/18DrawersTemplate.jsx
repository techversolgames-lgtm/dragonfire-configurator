...
const EighteenDrawersTemplate = ({
  ...
  const gltf = useGLTF("/models/dragonfire-tools/18D.glb");
  const sceneClone = useMemo(() => {
    ...
    clone.traverse((child) => {
      if (child.isMesh && child.material) {
        ...
        if (!isGhost && child.material.isMeshStandardMaterial && !skipCabinetBodyStyling) {
  ...
  const gltfWback = useGLTF("/models/dragonfire-tools/9ft_wback.glb");
  ...
  const wbackPosition = [0, 0, -boundingBox.depth / 2 + 0.0001];
  ...
  return (
    <>
      ...
      >
        ...
        {!isPartOfPackage && placement?.cabinetId === 5 && (
          <CabinetFeet
...
export default EighteenDrawersTemplate;
