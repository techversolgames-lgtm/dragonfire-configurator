...
const ObjectDraggingSystem = () => {
    ...
    const setIsOrbitControlsEnabled = useDragNDropStore((state) => state.setIsOrbitControlsEnabled);
    const setIsFloatingToolsVisible = useDragNDropStore((state) => state.setIsFloatingToolsVisible);
    ...
    function handlePointerDown(event) {
        ...
        if (filteredIntersects.length > 0) {
            ...
            if (activeSceneItem.parent && activeSceneItem.parent.type !== 'Scene') {
                parentMatrixWorldInverse.current.copy(activeSceneItem.parent.matrixWorld).invert();
            } else {
            ...
            let normal;
            const parentUuid = activeSceneItem.parent ? activeSceneItem.parent.uuid : null;
            ...
            dragPlane.current.setFromNormalAndCoplanarPoint(normal, worldPosition.current);
    ...
    function handlePointerMove(event) {
        ...
        if (raycaster.current.ray.intersectPlane(dragPlane.current, intersection.current)) {
            ...
            const localPos = newWorldPos.clone().applyMatrix4(parentMatrixWorldInverse.current);
...
export default ObjectDraggingSystem;